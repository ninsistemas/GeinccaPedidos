<template>
    <nav class="navbar navbar-expand navbar-light bg-dark topbar mb-4 static-top shadow">
      <div class="container">
          ...
          <p class="text-center" style="color: white"><small>{{ usuario }} <br> BCV: {{ factor | currency }}</small></p>
...
<script>
  export default {
    ...
    mounted(){
        ...
        this.usuario = datoslocales.spx_use_p+' - ' +datoslocales.spx_use_v
        //this.factorbcv = JSON.parse(localStorage.getItem('spx_factorbcv'));
        if(this.factorbcv){
            this.factor = this.factorbcv
        }
        else{
